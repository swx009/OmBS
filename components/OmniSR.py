...
class AdaptiveJointAttention(nn.Module):        #    自适应权重分配   注意力    模块
...
class OmniSR(nn.Module):
    def __init__(self, num_in_ch=3, num_out_ch=3, num_feat=64, **kwargs):
        ...
        # 初始化 AdaptiveJointAttention 和 EdgeEnhancement 模块
    ...
    def forward(self, x):
        ...
        # 应用 AdaptiveJointAttention 模块
        x = self.adaptive_joint_attention(x)

        x = self.residual_layer(x)

        # 应用 EdgeEnhancement 模块
